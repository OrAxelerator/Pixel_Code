...
>   Depuis racine du projet vous pouvez taper la commande : ``python3 pixel_code/__main__.py --debug`` (ou py/python selon votre machine) tout les logs seront envoyé dans le fichier ``debug.log``. A noter que pour le moment debug ne sont quasiment pas utilisé
