...
>  Pour changer les paramètres une fenêtre est accessible via la touche ``p`` mais peut-être que certains paramètres **ne sont pas encore modifiables** depuis cette interface, pour accéder totalement aux paramètres allez à : ``/pixel_code/data/parametres.json``.
